...
import {CustVehiclesService} from "../../../services/cust/custVehiclesService";
...
export class LandComponent extends AbstractDataModelContrlPublic<Vehicles> implements OnInit {
    ...
    constructor(public appSettings: AppSettings, private activatedRoute: ActivatedRoute, public service: CustVehiclesService,
        ...
        // this.formsManager.upsert('custSearch', this.filterComponentForm);
    }
    ...
    openSvPhoto(base64: string) {
        if (base64) {
            let photoObj: PhotoDTO = new PhotoDTO(base64, null, "", "image/jpg", 1);
            let dialogRef = this.dialog.open(SvItemPreviewDialogComponent, {
                data: photoObj,
                disableClose: false,
                width: '500px',
                autoFocus: true,
    ...
    doReservation(sv: Vehicles) {
        if (!this.isLoggedIn)
            this.msgService.showErrorMessageLocal("GENERIC.landing.reg.title.res");

        else {
            let dialogRef = this.dialog.open(ResvDialogComponent, {
                data: sv,
                disableClose: false,
                width: '700px',
                height: '500px',
